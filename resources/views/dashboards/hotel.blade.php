    ...
    <aside class="w-72 bg-dynamic-sidebar border-r border-gray-800 flex-shrink-0 hidden md:flex flex-col z-20 transition-all duration-300" :class="{'block absolute inset-y-0 left-0': sidebarOpen, 'hidden': !sidebarOpen}">
        ...
        <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            <button @click="activeTab = 'guest_requests'" 
                    class="w-full flex items-center gap-3 p-3 rounded-xl transition-all border-none cursor-pointer ltr:text-left rtl:text-right"
                    :class="activeTab === 'guest_requests' ? 'sidebar-active' : 'text-gray-400 hover:bg-white/5 hover:text-white bg-transparent'">
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-10 relative">
            ...
            <div x-show="activeTab === 'overview'" x-transition x-cloak>
                <div class="stats-bar mb-10">
                    <div class="stat-card border-dynamic shadow-[0_0_15px_color-mix(in_srgb,var(--dynamic-primary)_20%,transparent)] relative overflow-hidden bg-dynamic-sidebar">
                        <div class="absolute inset-0 bg-dynamic opacity-5"></div>
                        <div class="relative z-10">
                            ...
                            <div class="w-full bg-dynamic-main h-2 rounded-full overflow-hidden mt-4 shadow-inner">
                        ...
                        <table class="users-table w-full text-left rtl:text-right border-collapse">
                            <thead>
                                <tr class="border-b border-gray-800 text-gray-400">
                                    <th class="p-4 font-normal"><span class="en-text">Room</span><span class="ar-text">الغرفة</span></th>
                                    <th class="p-4 font-normal"><span class="en-text">Guest Name</span><span class="ar-text">اسم النزيل</span></th>
                                    <th class="p-4 font-normal"><span class="en-text">Request Type</span><span class="ar-text">نوع الطلب</span></th>
                                    <th class="p-4 font-normal"><span class="en-text">Status</span><span class="ar-text">الحالة</span></th>
                                    <th class="p-4 font-normal"><span class="en-text">Action</span><span class="ar-text">إجراء</span></th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($guestRequests as $request)
                                <tr class="hover:bg-white/5 transition-colors border-b border-gray-800/50 {{ $request->status === 'Pending' ? 'bg-red-500/5' : '' }}">
                                    ...
                                    <td class="p-4 text-gray-300">{{ $request->guest_name }}</td>
                                    <td class="p-4 text-white font-medium">{{ $request->request_type }}</td>
                                    <td class="p-4">
                                        @if($request->status === 'Pending')
                                            <span class="px-3 py-1.5 rounded-md bg-red-500/20 text-red-400 text-xs font-bold uppercase border border-red-500/30">Pending</span>
                                        @elseif($request->status === 'In Progress')
                                            <span class="px-3 py-1.5 rounded-md bg-dynamic/20 text-dynamic text-xs font-bold uppercase border border-dynamic/30 shadow-[0_0_10px_color-mix(in_srgb,var(--dynamic-primary)_20%,transparent)]">In Progress</span>
                                        @else
                                            <span class="px-3 py-1.5 rounded-md bg-emerald-500/20 text-emerald-400 text-xs font-bold uppercase border border-emerald-500/30">Resolved</span>
                                        @endif
                                    </td>
                                    <td class="p-4">
                                        <select class="glass-input-premium w-full text-sm bg-dynamic-main border border-gray-700 text-white shadow-inner" @change="updateStatus({{ $request->id }}, $event.target.value)">
