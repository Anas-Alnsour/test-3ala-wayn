@php
    // Fetch restaurant's active offers
    ...
    // Live Orders with Wiki titles for imagery
    $liveOrders = [
        (object)['id' => '#ORD-001', 'guest' => 'John Doe', 'deal' => 'Mansaf', 'wiki_title' => 'Mansaf', 'time' => '10m ago', 'status' => 'Arriving'],
        (object)['id' => '#ORD-002', 'guest' => 'Sarah M.', 'deal' => 'Falafel Platter', 'wiki_title' => 'Falafel', 'time' => '25m ago', 'status' => 'Seated'],
        (object)['id' => '#ORD-003', 'guest' => 'Ahmad R.', 'deal' => 'Knafeh', 'wiki_title' => 'Knafeh', 'time' => '1h ago', 'status' => 'Completed'],
    ];
@endphp
    ...
    <aside class="w-72 bg-[#1a0f0a] border-r border-[#2e1c12] flex-shrink-0 hidden md:flex flex-col z-40 transition-all duration-300 shadow-[4px_0_30px_rgba(0,0,0,0.8)]"
        ...
        <nav class="flex-1 p-5 space-y-3 overflow-y-auto">
            <button @click="activeTab = 'overview'"
                    ...
                    :class="activeTab === 'overview' ? 'bg-[#F4A261]/10 border-[#F4A261]/30 text-[#F4A261] shadow-2xl' : 'text-gray-400 hover:bg-[#2e1c12]/50 hover:text-white'">
            ...
            <button @click="activeTab = 'live_orders'"
                    ...
                    :class="activeTab === 'live_orders' ? 'bg-[#F4A261]/10 border-[#F4A261]/30 text-[#F4A261] shadow-2xl' : 'text-gray-400 hover:bg-[#2e1c12]/50 hover:text-white'">
        ...
        <div class="p-6 border-t border-[#2e1c12]">
        ...
        <header class="h-24 bg-[#120a07]/90 backdrop-blur-xl border-b border-[#2e1c12] flex items-center justify-between px-6 lg:px-10 z-10 sticky top-0 shadow-lg">
             <div class="flex items-center gap-4">
                 ...
                 <h2 class="text-3xl font-black text-white hidden sm:block">
                     ...
                     <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#F4A261] to-[#E76F51]">{{ Auth::user()->name }}</span> 👨‍🍳
                 </h2>
             </div>
             <div class="flex items-center gap-6">
                <button @click="toggleLang()" class="w-12 h-12 rounded-2xl border border-[#2e1c12] bg-[#1a0f0a] hover:border-[#F4A261] text-sm font-bold text-gray-300 cursor-pointer transition-all">
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-10 relative bg-transparent">
            <div x-show="activeTab === 'overview'" x-transition x-cloak>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                    <div class="bg-[#1a0f0a] border border-[#2e1c12] p-8 rounded-[2rem] group hover:border-[#F4A261]/50 transition-all">
                    ...
                    <div class="bg-gradient-to-br from-[#F4A261] to-[#E76F51] p-8 rounded-[2rem] text-black">
                        <p class="font-black uppercase text-xs tracking-widest mb-2">Total Saved</p>
                        <h3 class="text-4xl font-black">{{ $totalSaved }} JOD</h3>
                    </div>
                </div>

                <div class="bg-[#1a0f0a] border border-[#2e1c12] rounded-[2.5rem] p-8">
                    ...
                    <div class="space-y-6">
                         @foreach($liveOrders as $ord)
                            ...
                            <div class="flex items-center gap-6 p-4 rounded-2xl bg-white/5 border border-white/5">
                                <div class="w-14 h-14 rounded-xl relative overflow-hidden" x-wiki-image="'{{ $ord->wiki_title }}'">
                                    <img src="" class="w-full h-full object-cover">
                                </div>
                                <div class="flex-1">
                                    <p class="text-white font-bold">{{ $ord->guest }} enjoyed {{ $ord->deal }}</p>
            ...
            <div x-show="activeTab === 'live_orders'" x-transition x-cloak>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                     @foreach($liveOrders as $ord)
                        @if($ord->status !== 'Completed')
                        <div class="bg-[#1a0f0a] border border-[#2e1c12] rounded-[2.5rem] overflow-hidden group hover:border-[#F4A261]/50 transition-all flex flex-col">
                            <div class="h-48 relative" x-wiki-image="'{{ $ord->wiki_title }}'">
                                ...
                                <div class="absolute top-6 left-6 px-4 py-2 bg-[#F4A261] text-black font-black rounded-xl text-xs">
                                    {{ $ord->status }}
                                </div>
                            </div>
                            <div class="p-8">
                                ...
                                <button @click="showToast('Order Updated!', 'bg-[#F4A261] text-black')" class="w-full py-4 bg-white/5 hover:bg-[#F4A261] border border-white/10 hover:border-transparent text-white hover:text-black font-black rounded-2xl transition-all cursor-pointer">MARK AS SEATED</button>
                            </div>
                        </div>
                        @endif
                     @endforeach
