@php
    ...
    $liveDeals = [
        (object)['id'=>1, 'title' => '50% Off Wadi Rum', 'wiki_title' => 'Wadi_Rum', 'location' => 'Wadi Rum', 'discount' => '50%'],
        (object)['id'=>2, 'title' => 'Friday Mansaf Deal', 'wiki_title' => 'Mansaf', 'location' => 'Amman', 'discount' => '25%'],
        (object)['id'=>3, 'title' => 'Ajloun Cable Car', 'wiki_title' => 'Ajloun_Castle', 'location' => 'Ajloun', 'discount' => '15%'],
    ];
    $liveDealsCount = count($liveDeals);
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        <header class="h-24 bg-[#121212]/90 backdrop-blur-xl border-b border-[#2a2a2a] flex items-center justify-between px-6 lg:px-10 z-10 sticky top-0 shadow-lg">
             <div class="flex items-center gap-4">
                 ...
                 <h2 class="text-3xl font-black text-white hidden sm:block">
                     ...
                     <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#F5C518] to-[#D4AF37]">{{ Auth::user()->name }}</span> 👑
                 </h2>
             </div>
             <div class="flex items-center gap-6">
                ...
                <form method="POST" action="{{ route('logout') }}" class="m-0">
                    @csrf
                    <button type="submit" class="p-3 text-red-500 hover:bg-red-500/10 rounded-2xl transition-colors cursor-pointer bg-transparent border-none">
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-10 relative bg-transparent">
            <div x-show="activeTab === 'overview'" x-transition x-cloak>
                ...
                <div class="bg-[#121212] border border-[#2a2a2a] rounded-[2.5rem] p-8">
            ...
            <div x-show="activeTab === 'approvals'" x-transition x-cloak>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div id="gem-{{ $gem->id }}" class="bg-[#121212] border border-[#2a2a2a] rounded-[2.5rem] overflow-hidden flex flex-col hover:border-[#F5C518]/50 transition-all group">
                        <div class="h-64 relative" x-wiki-image="'{{ $gem->wiki_title ?? $gem->city->wiki_title }}'">
                            ...
                            <div class="absolute top-6 left-6 px-4 py-2 bg-[#F5C518] text-black font-black rounded-xl text-xs uppercase">
                                {{ $gem->city->name }}
                            </div>
                        </div>
                        <div class="p-8">
...
<p class="text-sm text-gray-500">Submitted by
    <span class="text-white font-bold">{{ $gem->submitter?->name ?? 'Wayn System' }}</span>
                    ...
                    <div class="col-span-full p-20 text-center bg-[#121212] border border-dashed border-[#2a2a2a] rounded-[2.5rem]">
                        <p class="text-gray-500 font-black text-xl tracking-widest uppercase">The Throne is Silent. No pending tasks.</p>
                    </div>
                    @endforelse
                </div>
            </div>
        ...
        return {
            ...
            processApproval(id, action) {
                ...
                fetch(`/admin/approvals/${id}/${action}`, {
                    ...
                    this.showToast(action === 'approve' ? '✅ Action Approved' : '❌ Submission Rejected', 'bg-[#F5C518] text-black border-none');
                });
            },
