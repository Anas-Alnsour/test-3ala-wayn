@php
    ...
    // Upcoming Tours with Direct Wikimedia fallback links for Guaranteed Quality
    $upcomingTours = [
        (object)[
            'id' => 1,
    ...
    $payouts = [
        (object)['id'=>'TRX-9821', 'amount' => '150 JOD', 'date' => 'Oct 12, 2026', 'status' => 'Paid'],
...
<div class="theme-assistant flex h-screen w-full bg-[#0d0a09] text-[#f5f0e6] font-sans overflow-hidden transition-colors duration-500"
    ...
    <aside class="w-72 bg-[#16110f] border-r border-[#2d221e] flex-shrink-0 hidden md:flex flex-col z-[60] transition-all duration-300 shadow-[4px_0_30px_var(--dynamic-glow)]"
        ...
        <nav class="flex-1 p-5 space-y-3 overflow-y-auto">
            <button @click="activeTab = 'tours'"
                    ...
                    :class="activeTab === 'tours' ? 'bg-[#E76F51]/10 border-[#E76F51]/30 text-[#E76F51] shadow-[0_0_15px_var(--dynamic-glow)]' : 'text-gray-400 hover:bg-[#1f1815] hover:text-white'">
            ...
            <button @click="activeTab = 'financials'"
                    ...
                    :class="activeTab === 'financials' ? 'bg-[#E76F51]/10 border-[#E76F51]/30 text-[#E76F51] shadow-[0_0_15px_var(--dynamic-glow)]' : 'text-gray-400 hover:bg-[#1f1815] hover:text-white'">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        <header class="h-24 bg-[#120e0c]/90 backdrop-blur-xl border-b border-[#2d221e] flex items-center justify-between px-6 lg:px-10 z-50 sticky top-0 shadow-lg">
             <div class="flex items-center gap-4">
             ...
             <div class="flex items-center gap-6">
                <button @click="toggleLang()" class="w-12 h-12 rounded-2xl border border-[#2d221e] bg-[#1a1311] hover:border-[#E76F51] text-sm font-bold text-gray-200 cursor-pointer transition-all shadow-inner">
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-10 relative bg-transparent z-10">
            <div x-show="activeTab === 'tours'" x-transition x-cloak>
                 <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                    <div class="bg-[#16110f] border border-[#2d221e] p-8 rounded-[2rem] group hover:border-[#E76F51]/50 transition-all shadow-lg">
                    ...
                    <div class="bg-gradient-to-br from-[#E76F51] to-[#F4A261] p-8 rounded-[2rem] text-black shadow-lg">
                    ...
                    <div class="bg-[#16110f] border border-[#2d221e] rounded-[2.5rem] overflow-hidden group hover:border-[#E76F51]/50 transition-all flex flex-col lg:flex-row shadow-2xl">
                        ...
                        <div class="flex-1 p-8 lg:p-12 flex flex-col justify-center">
                            ...
                            <div class="flex gap-4">
                                <button @click="showToast(language === 'ar' ? '🚀 بدأت الرحلة!' : 'Tour Started! 🚀', 'bg-[#E76F51] text-black border-none')" class="px-8 py-4 bg-white/5 hover:bg-[#E76F51] border border-white/10 hover:border-transparent text-white hover:text-black font-black rounded-2xl transition-all cursor-pointer shadow-lg">START MISSION</button>
            ...
            <div x-show="activeTab === 'financials'" x-transition x-cloak>
                 <div class="mb-10 text-center max-w-2xl mx-auto">
                    <h2 class="text-5xl font-black text-white mb-4" x-text="language === 'ar' ? 'خزنة الكابتن' : 'The Captain\'s Vault'"></h2>
                    <p class="text-gray-400 text-lg" x-text="language === 'ar' ? 'أرباحك التي جنيتها من إرشاد المستكشفين في أرجاء الأردن.' : 'Your hard-earned wealth from guiding explorers through Jordan\'s soul.'"></p>
                </div>
